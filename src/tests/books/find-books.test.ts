...
import { API_BOOKS_URL, createBookWithUser } from '@tests/helpers';

describe('GET /books', () => {
  describe('Public access', () => {
    test('Should not require authentication', async () => {
      const response = await request(getTestApp()).get(API_BOOKS_URL);
  ...
  test('Should allow searching by title', async () => {
    ...
    const response = await request(getTestApp()).get(`${API_BOOKS_URL}/?search=${title}`);
  ...
  test('Should allow searching by author', async () => {
    ...
    const response = await request(getTestApp()).get(`${API_BOOKS_URL}/?search=${author}`);
    const validateResponse = findBooksResponseSchema.parse(response.body);
  ...
  test('Should respect page and limit query params', async () => {
    ...
    const response = await request(getTestApp()).get(`${API_BOOKS_URL}?page=1&limit=5`);
    const validateResponse = findBooksResponseSchema.parse(response.body);
  ...
  test('Should return empty array when page exceeds total pages', async () => {
    ...
    const response = await request(getTestApp()).get(`${API_BOOKS_URL}?page=999&limit=5`);
    const validateResponse = findBooksResponseSchema.parse(response.body);
  ...
  test('Should fallback to defaults when page or limit is invalid', async () => {
    const response = await request(getTestApp()).get(`${API_BOOKS_URL}?page=-1&limit=abc`);
    const validateResponse = findBooksResponseSchema.parse(response.body);
