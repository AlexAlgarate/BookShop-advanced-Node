...
describe('GET /books', () => {
  ...
  test('Should allow searching by title', async () => {
    ...
    const title = newRandomBook.body.content.title;

    const response = await request(app).get(`${BOOKS_URL}/?search=${title}`);

    expect(response.status).toBe(200);
    expect(response.body.content.some((book: any) => book.title.includes(title))).toBe(true);
  });
  ...
  test.todo('Should return paginated results (default pagination');
  ...
  test('Should return consistent book shape', async () => {
    ...
    const book = response.body.content;
